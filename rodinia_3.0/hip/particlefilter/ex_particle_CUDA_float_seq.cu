...
__device__ double calcLikelihoodSum(unsigned char * I, int * ind, int numOnes, int index) {
    ...
    int x;
    for (x = 0; x < numOnes; x++)
        likelihoodSum += (pow((double) (I[ind[index * numOnes + x]] - 100), (double) 2) - pow((double) (I[ind[index * numOnes + x]] - 228), (double) 2)) / 50.0;
...
__global__ void likelihood_kernel(double * arrayX, double * arrayY, double * xj, double * yj, double * CDF, int * ind, int * objxy, double * likelihood, unsigned char * I, double * u, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, double* partial_sums) {
    ...
    if (i < Nparticles) {
        for (y = 0; y < countOnes; y++) {
            ...
            ind[i * countOnes + y] = (int)fabs( (float)(indX * IszY * Nfr + indY * Nfr + k));
...
void strelDisk(int * disk, int radius) {
    ...
    for (x = 0; x < diameter; x++) {
        for (y = 0; y < diameter; y++) {
            double distance = sqrt(pow((double) (x - radius + 1), (double) 2) + pow((double) (y - radius + 1), (double) 2));
            if (distance < radius)
                disk[x * diameter + y] = 1;
        }
    }
...
void dilate_matrix(unsigned char * matrix, int posX, int posY, int posZ, int dimX, int dimY, int dimZ, int error) {
    ...
    for (x = startX; x < endX; x++) {
        for (y = startY; y < endY; y++) {
            double distance = sqrt(pow((double) (x - posX), (double) 2) + pow((double) (y - posY), (double) 2));
            if (distance < error)
...
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    //fprintf(stderr, "TIME TO SEND TO GPU: %f\n", elapsed_time(send_start, send_end));
    int num_blocks = ceil((double) Nparticles / (double) threads_per_block);
    ...
    printf("XE: %lf\n", xe);
    printf("YE: %lf\n", ye);
    double distance = sqrt(pow((double) (xe - (int) roundDouble(IszY / 2.0)), (double) 2) + pow((double) (ye - (int) roundDouble(IszX / 2.0)), (double) 2));
    printf("%lf\n", distance);
...
int main(int argc, char * argv[]) {
    ...
    //fprintf(stderr,"VIDEO SEQUENCE TOOK %f\n", elapsed_time(start, endVideoSequence));
    //call particle filter
    particleFilter(I, IszX, IszY, Nfr, seed, Nparticles);
    long long endParticleFilter = get_time();
    //fprintf(stderr,"PARTICLE FILTER TOOK %f\n", elapsed_time(endVideoSequence, endParticleFilter));
    //fprintf(stderr,"ENTIRE PROGRAM TOOK %f\n", elapsed_time(start, endParticleFilter));
